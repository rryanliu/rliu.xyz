...
A stroke of inspiration hit me and I solved it minutes before I dropped him off.
...
Since $r^{-1} r^{\prime}$ is in $\mathbb{Z} / 8 \mathbb{Z}$, we can flip any coin in this row
...
So now, setting aside groups groups, Harry can just
flip the coin in row $r \oplus r^{\prime}$ and $c \oplus c^{\prime}$.
